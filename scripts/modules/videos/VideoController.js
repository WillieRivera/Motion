...
function VideoController(uiGridExporterConstants, uiGridExporterService, $timeout, $uibModal, $scope, $rootScope, $localStorage, $http, $filter, $location, helpers, EventService, UserService, VideoService, ProjectsService, CamerasService, AwsService) {
    ...
    $scope.gridOptions = {
        columnDefs: [
            ...
            { 
                ...
                cellTemplate: '<div class="progress"><div class="progress-bar" role="progressbar" aria-valuenow="70" aria-valuemin="0" aria-valuemax="100" style="width:{{row.entity.progress}}%"><span class="sr-only">70% Complete</span></div></div>'
            },
            {
                field: 'play',
                displayName: 'Play',
                ...
                cellTemplate: '<span class="input-group-addon" ng-click="grid.appScope.videoVM.openVideoAnalysis(row.entity)"><span class="glyphicon glyphicon-play-circle" aria-hidden="true"></span></span>'
            },
            {
                name: 'Edit',
                enableHiding: true,
            ...
            },
            {
                ...
                visible:true,
                enableHiding:true,
                cellTemplate: '<label class="switch"><input type="checkbox" ng-checked="{{row.entity.status}}" ng-click="grid.appScope.videoVM.toggleVideoStatus(row.entity);"><span class="slider round"></span></label>'
            }
        ],
    ...
    vm.openVideoAnalysis = function(video) {
        $rootScope.selectedVideo=video;
        $location.path('/dashboard')
    }

    vm.toggleVideoStatus = function(video) {
        VideoService.toggleVideoStatus(video.videoId);
    }
